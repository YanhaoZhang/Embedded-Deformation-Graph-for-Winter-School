...
obj.Jacobian_whole = zeros(obj.num_nodes*(6+3*obj.num_connection) ...  % ednode
                   + dim_obs*obj.num_control_pts_prior, ...   % observation term
                   obj.num_nodes * 12);
...
for i=1:obj.num_control_pts_prior
    % calculate nodes before transform using model2camera pose, which means the point_rot_trans is in model frame
    vertex_prior_I = obj.control_pts_prior(:,i);
    weight_prior_I = obj.control_pts_prior_weight(i,:);
    weight_id      = obj.control_pts_prior_weight_id(i,:);
