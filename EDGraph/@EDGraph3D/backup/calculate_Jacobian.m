...
obj.Jacobian_whole = zeros(obj.num_nodes*(6+3*obj.num_connection) ...  % ednode
                   + dim_obs*obj.num_controlCenterline_allFrames, ...   % observation term
                   obj.num_nodes * 12);       %Yanhao20190115: 只用最后一个点的x y z　其他都用
...
%     node_tj = obj.node_translation(:,i);
    ...
    for i = 1:obj.num_connection
        node_id = weight_id(i+1);                  % j+1 不是 j
        node_gi = obj.node_position(:,node_id);
%         node_tk = obj.node_translation(:,node_id);
        d_t = node_gi - node_gj;
        J_x_node(6+3*(i-1)+1:6+3*i,:) = ...                                                
            sigma_reg * [d_t(1)*eye(3,3),d_t(2)*eye(3,3),d_t(3)*eye(3,3),eye(3,3)];  %embedded graph文章中说weight=1

        % Ri and ti:
        potision_k = node_id * 12 - 2;
        obj.Jacobian_whole(position_x+6+3*(i-1):position_x+6+3*i-1,potision_k:potision_k+2) = sigma_reg * (-1)*eye(3,3);      % 0.6哪来的？？？
...
%             node_tj = obj.node_translation(:,node_id);
            ...
            % store it for jacoian
            obj.Jacobian_whole(position_x+dim_obs*(observation_ii-1)+1:position_x+dim_obs*observation_ii, (node_id-1)*12+1:node_id*12) = sigma_obs * Jacobian_tem;
        end
    end
end


%% use original jacobian to check
obj.Jacobian_whole = sparse(obj.Jacobian_whole);
end